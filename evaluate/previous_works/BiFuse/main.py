...
#                only_dep = 1 - only_dep
                only_dep = cv2.cvtColor(cat_dep[upper:lower],cv2.COLOR_RGB2GRAY)
                vmax = np.percentile(only_dep, 95)
                normalizer = mpl.colors.Normalize(vmin=only_dep.min(), vmax=vmax)
                mapper = cm.ScalarMappable(norm=normalizer, cmap='magma')
                only_dep = (mapper.to_rgba(only_dep)[:, :, :3] * 255).astype(np.uint8)

                plt.imsave('My_Test_Result/Depth_magma%.3d.jpg'%count, only_dep, cmap='magma')
...
#                only_dep = np.array(Image.open('My_Test_Result/Depth%.3d.jpg'%count).convert('LA'))
#                vmax = np.percentile(only_dep, 95)
#                normalizer = mpl.colors.Normalize(vmin=only_dep.min(), vmax=vmax)
#                mapper = cm.ScalarMappable(norm=normalizer, cmap='magma')
#                only_dep = (mapper.to_rgba(only_dep)[:, :, :3] * 255).astype(np.uint8)
#                plt.imsave('My_Test_Result/DepthM%.3d.jpg'%count, only_dep, cmap='magma')
